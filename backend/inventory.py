...
def C_lote(lot_id: str, cantidad: int, dias_caducidad: int, product_name: str = "Producto"):
    """
    ...
    for _ in range(cantidad):
        ...
        cur.execute(
            "INSERT INTO inventory (lot_id, id_individual, caducidad, product_name) VALUES (?, ?, ?, ?)",
            (lot_id, id_individual, fecha_cad, product_name)
        )
        productos.append({
...
def R_lote(lot_id: str):
    ...
    productos = [
        {"id_individual": r[1], "caducidad": r[2], "product_name": r[3]}
        for r in rows
    ]
    lote_data = {
        "lot_id": lot_id,
        "cantidad": len(productos),
        "product_name": productos[0]["product_name"] if productos else "Unknown",
...
def D_lote(identifier: str):
    ...
    inventario = [
        {"lot_id": r[0], "id_individual": r[1], "caducidad": r[2], "product_name": r[3]}
        for r in rows
    ]
    return inventario